  ...
  cardsData.forEach((card) => {
    ...
    cardElement.innerHTML = `
              <div class="flip-card-inner">
                  <div id="card-${card.id}" class="flip-card-front bg-[#1f57bf] dark:bg-gray-800 text-white dark:text-gray-100 p-6 flex flex-col items-center justify-center rounded-lg">
                      <i class="fa-solid ${card.icon} text-[3rem] mb-5"></i>
                      <p class="text-2xl font-bold" data-key="${card.dataKeyTitle}">${card.title}</p>
                  </div>
                  <div class="flip-card-back bg-[#1f57bf] dark:bg-gray-800 text-white dark:text-gray-100 p-6 flex flex-col items-center justify-center rounded-lg">
                      <p class="text-2xl font-bold" data-key="${card.dataKeyTitle}">${card.title}</p>
                      <p class="py-4" data-key="${card.dataKeyDescription}">${card.description}</p>
                      <a href='${card.href}' class="mt-4 py-2 px-4 bg-[#1f57bf] dark:bg-gray-800 text-white dark:text-gray-100 border-2 border-white dark:border-gray-600 rounded" data-key="learn_more">Daha ətraflı</a>
                  </div>
              </div>
          `;
